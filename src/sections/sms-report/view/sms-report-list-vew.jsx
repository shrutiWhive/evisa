...
export function SmsReportListView() {
  const table = useTable();

  const dispatch = useAppDispatch();

  const { smsReport } = useAppSelector(selectReport);

  useEffect(() => {
    dispatch(fetchSmsRequest());
  }, [dispatch]);
  ...
  const flattenedReport = smsReport?.sms_debits || [];
      ...
      <CardHeader
        ...
        action={
          ...
          >
            ...
            <Label color="primary">
              Total SMS debit: {smsReport?.total_sms_debits || 0}
            </Label>
          </Box>
        }
      />
